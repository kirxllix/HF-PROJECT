        ...
        const productsWithStock = await Product.aggregate([
            ...
            // Paso 4: Proyectar el resultado y calcular el stock visible
            {
                $project: {
                    ...
                    descripcion: 1,
                    categoria: 1,
                    habilitarVentaOnline: 1,
                    variaciones: 1,
                    // Lógica de Negocio: El stock visible es el t1_mostrador del turnoMañana
                    stockDisponible: {
                        $cond: {
                            if: { $and: [ '$habilitarVentaOnline', '$inventoryData.turnoManana' ] },
                            then: '$inventoryData.turnoManana.t1_mostrador',
                            else: 0 // Si no hay inventario o no es turno mañana, stock 0
                        }
                    }
                }
            }
...
exports.solicitarPedido = async (req, res) => {
    ...
    for (const item of items) {
        ...
        // a) Validar que exista y sea vendible en línea
        if (!productDoc || !productDoc.habilitarVentaOnline) {
            return res.status(400).json({ message: `Producto no encontrado o no apto para venta en línea.` });
        }
