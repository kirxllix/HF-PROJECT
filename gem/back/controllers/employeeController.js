...
// @access  Private/Employee
// @desc    Obtener inventario con ventas separadas por turno
// @route   GET /api/v1/employee/inventory/today
exports.getTodayInventory = async (req, res) => {
    ...
    try {
        // 1. Obtener registro de inventario para saber la hora de corte
        const inventoryRecord = await Inventory.findOne({ IDSucursal, fecha: today });
        
        // Determinar hora de corte (Si se cerró el turno mañana, usamos esa hora)
        let cutoffTime = new Date(); 
        cutoffTime.setHours(23, 59, 59, 999); // Por defecto, todo es mañana si no se ha cerrado

        if (inventoryRecord && inventoryRecord.turnoManana && inventoryRecord.turnoManana.horaRegistro_Cierre) {
            cutoffTime = inventoryRecord.turnoManana.horaRegistro_Cierre;
        }

        // 2. Obtener registros (para el mapeo)
        const inventoryRecords = await Inventory.find({ IDSucursal, fecha: today })
            .populate('IDProducto', 'nombreProducto variaciones');

        // 3. Aggregation para sumar ventas separadas por turno
        const onlineSales = await Order.aggregate([
            ...
            {
                $group: {
                    _id: { $ifNull: ['$items.IDProducto', '$items.IDPRODUCTO'] }, 
                    // Sumar si la fecha es MENOR o IGUAL al corte (Mañana)
                    ventasManana: { 
                        $sum: { 
                            $cond: [{ $lte: ["$fecha", cutoffTime] }, '$items.cantidad', 0] 
                        } 
                    },
                    // Sumar si la fecha es MAYOR al corte (Noche)
                    ventasNoche: { 
                        $sum: { 
                            $cond: [{ $gt: ["$fecha", cutoffTime] }, '$items.cantidad', 0] 
                        } 
                    }
                }
            }
        ...
        onlineSales.forEach(sale => {
            if (sale._id) {
                ...
                if (inventoryMap[productId]) {
                    inventoryMap[productId].ventasOnlineManana = sale.ventasManana;
                    inventoryMap[productId].ventasOnlineNoche = sale.ventasNoche;
                }
            }
        });
        ...
        console.error("Error en getTodayInventory:", error); 
...
// ... (inicio del archivo e importaciones)
...
exports.getPendingOrdersBySucursal = async (req, res) => {
    ...
    try {
        const orders = await Order.find({ 
            ...
            tipoPedido: 'menudeo' // <-- AÑADE ESTA LÍNEA
        })
        .sort({ fecha: 1 })
        .populate('IDUSUARIO', 'nombrePila primerApell'); 
